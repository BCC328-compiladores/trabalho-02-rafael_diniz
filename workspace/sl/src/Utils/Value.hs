...
(.-.) :: MonadError String m => Value -> Value -> m Value 
(VInt n1) .-. (VInt n2) = pure (VInt (n1 - n2))
v1 .-. v2 = throwError $ unlines ["Cannot subtract:", pretty v1, "with", pretty v2]
...
(VInt n1) ./. (VInt n2) = if n2 == 0 
                         ...
                         else pure (VInt (n1 `div` n2))
v1 ./. v2 = throwError $ unlines ["Cannot divide:", pretty v1, "with", pretty v2]

(.%.) :: MonadError String m => Value -> Value -> m Value 
(VInt n1) .%. (VInt n2) = if n2 == 0 
                         then throwError "Modulo by zero"
                         else pure (VInt (n1 `mod` n2))
v1 .%. v2 = throwError $ unlines ["Cannot modulo:", pretty v1, "with", pretty v2]
...
(.&.) :: MonadError String m => Value -> Value -> m Value
(VBool b1) .&. (VBool b2) = pure (VBool (b1 && b2))
v1 .&. v2 = throwError $ unlines ["Cannot conjunct:", pretty v1, "with", pretty v2]
